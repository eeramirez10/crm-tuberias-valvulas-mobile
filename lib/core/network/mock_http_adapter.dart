  ...
  Future<Map<String, dynamic>> request({
    ...
    if (path == ApiEndpoints.leads && method == HttpMethod.post) {
      return _dataStore.createLead(
        ...
        industrialSector: (data?['industrial_sector'] as String?) ?? '',
        creditStatus: (data?['credit_status'] as String?) ?? '',
        projectState: (data?['project_state'] as String?) ?? '',
        projectCity: (data?['project_city'] as String?) ?? '',
        projectLatitude: (data?['project_latitude'] as num?)?.toDouble(),
        projectLongitude: (data?['project_longitude'] as num?)?.toDouble(),
        requiredDeliveryTime:
            (data?['required_delivery_time'] as String?) ?? '',
        mainCompetitor: (data?['main_competitor'] as String?) ?? '',
        material: (data?['material'] as String?) ?? '',
        schedule: (data?['schedule'] as String?) ?? '',
        nominalDiameter: (data?['nominal_diameter'] as String?) ?? '',
        endType: (data?['end_type'] as String?) ?? '',
        valveType: (data?['valve_type'] as String?) ?? '',
        pressureClass: (data?['pressure_class'] as String?) ?? '',
        standard: (data?['standard'] as String?) ?? '',
        lossReason: (data?['loss_reason'] as String?) ?? '',
